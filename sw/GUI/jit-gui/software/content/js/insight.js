...
{
	// Close the ports when the window is closing
	if (eccRoot!= null)
	{
		eccRoot.close();
	}
...
function initThing(credAccessKey, credSecretKey)
{
	...
	// If this is the active.html page, set the thingName in the title bar.
	if (document.URL != null && document.URL.toString().search('active.html') >= 0)
	{
		// Set the thingName in the title bar.
		if (document.getElementById("thingname") != null)
			document.getElementById("thingname").innerHTML = "AWS Thing Name: " + credThingName;
...
{
	// Check the variables
	if (port.comName == null) return false;
	if (port.pnpId== null) return false;
	if (port.manufacturer== null) return false;
...
{
	// Make certain we don't dereference an undefined string
	if (str == null) return '';
...
function hexAsciiToByteArray(hexAscii)
{
	...
	{
		var hexByte = Number(parseInt(hex.substr(i, 2), 16));
		arr.push(hexByte);
	}
	return arr;
}
...
function receiveThingBytes(reply)
{
	...
	case "KitName":
		...
		if (document.URL != null && document.URL.toString().search('securething.html') >= 0)
		{
			awsThingVersion = kitVersion(data);
			var kitName = name + ' (AWS v' + `${awsThingVersion[0]}.${awsThingVersion[1]}.${awsThingVersion[2]})`;
			document.getElementById('awsthingstring').value = kitName ;
...
/*
// Move SaveWiFiCred to happen only upon awsRegisterThing success
...
function receiveThingError(error)
{
	...
	alert('Thing Communication Error: ' + error);
	console.log('Thing Communication Error: ' + error);
	//eccThing.close();
	//eccThing = null;
}
...
function kitVersion(data)
{
	// The response from the kit will be in the form:
	// ASCII Hex version.  3 bytes: <major><minor><build>
	// Example: 010005
	// Convert this string to a byte array
	// Example: {0x01, 0x00, 0x05}

	// Extract the data bytes
	return hexAsciiToByteArray(data);
}
...
function prepareThing()
{
	...
	// Check the firmware version before continuing
	if (awsThingVersion != null && awsThingVersion.length > 2)
	{
		var awsMinVer = 1*100 + 0*10 + 6;
		var awsVer = awsThingVersion[0]*100 + awsThingVersion[1]*10 + awsThingVersion[2];
		if (awsVer < awsMinVer)
		{
			alert("Please update your AWS Thing firmware at\nhttps://github.com/MicrochipTech/AWS-Secure-Insight/tree/master/sw/FW");
			return;
		}
	}
...
function awsRegisterSigner()
{
	...
	awsIot.registerCACertificate(params, function(err, data)
	{
		...
		{
			// an error occurred
			console.log("Failed to register Signing Module\n", err, err.stack);
		}
		else
		{
			// successful response
			//console.log(data);
			console.log("Signing Module Registration Successful!\n");
		}
	});
}
...
function awsRegisterThing()
{
	if (useJitr)
	{
		// only send the aws:w() command
		// Save the WiFi credentials after a successful completion of attaching the policy to the cert
		sendThingBytes('aws:sw(' + strToHexAscii(credWiFiSsid) + ',' + strToHexAscii(credWiFiPassword) + ')\n');
		thingState = "SaveWiFiCred";
	}
	else
	{
		// Call the BYOC version to register the Thing certificate
		awsRegisterThingBYOC();
	}
}

function awsRegisterThingBYOC()
{
	// Print the certificates
	console.log('thingCert:\n' + thingCert);
	console.log('signerCert:\n' + signerCert);
	...
	awsIot.registerCertificate(params, function(err, data)
	{
		if (err)
		{
			console.log("Signer still registering... AWS failed to register \"Thing\" certificate\n");
			console.log("AWS failed to register \"Thing\" certificate", err, err.stack);
		}
		else
		{
			// successful response
			console.log("Thing successfuly registered\n");
			//console.log(data);
			thingCert = data;
			awsGetPolicy();
		}
...
function awsGetPolicy()
{
	...
	awsIot.getPolicy(params, function(err, data)
	{
		...
		else
		{
			// successful response
			//console.log(data);
			console.log("Get Policy Succesful!\n");
			policy= data;
...
function awsAttachPolicy()
{
	...
	}, (err, data) =>
	{
		...
		if (err && (!err.code || err.code !== 'ResourceAlreadyExistsException'))
		{
			//alert("Failed to attach Policy to \"Thing\" certificate");
			console.log("Failed to attach Policy to \"Thing\" certificate/n" + err);
			return;
		}
		console.log(data);
