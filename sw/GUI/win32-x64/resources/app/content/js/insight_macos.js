...
/*
// TODO: Remove this hard-coded account information
var awsThingName = "awsSecureThing"
var awsRegion = "us-west-2";
var awsAccessKey = "AKIAIW5N2D3ZXLH6KD5Q";
var awsSecretKey = "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";
//var awsEndPoint = "a3adakhi3icyv9.iot.us-west-2.amazonaws.com";
var awsEndPoint = "iot.us-west-2.amazonaws.com";
var awsApiVersion = "2015-05-28";
var awsThingName = "AwsLightTest";

// WiFi credentials
var wifiSsid = "SecureAP";
var wifiPw = "SecureAP";
*/
...
function checkSerialPort(port)
{
	...
	if (port.manufacturer == null || port.productId ==null || port.vendorId == null)
		return false;
	...
	if (port.vendorId == '0x03eb' && port.productId == '0x2404' && eccThing == null)
	{
...
function receiveThingBytes(reply)
{
	...
	case "GetThingCert":
		...
		thingCert = hexAsciiToStr(kitData(thingReply));
...
{
	console.log('Thing Error Returned: '+ error);
}
...
function sendRootBytes(data)
{
	...
	var bytes = map.call(strData, function(x) { return x.charCodeAt(0); });
	var prefix = '_';
	var prefixByte = prefix.charCodeAt(0);
	...
	if (data.length > maxHidBytes)
	{
 		...
 		do
		{
			...
			//hidChunk.splice(0, 0, prefixByte);
...
function receiveRootBytes(reply)
{
	...
	case "GetRootCert":
		...
		var checkCert = {certId: RootCertId, partialCert: kitData(rootReply), certificate: rootCert };
		var fullCert = checkCertReply(checkCert);
...
{
	console.log('Root Error Returned: '+ error);
}
...
function receiveSignerBytes(reply)
{
	...
	case "GetSignerCert":
		...
		var checkCert = {certId: SignerCertId, partialCert: kitData(signerReply), certificate: signerCert };
		var fullCert = checkCertReply(checkCert);
	...
	case "GetVerifyCert":
		...
		var checkCert = {certId: VerifyCertId, partialCert: kitData(signerReply), certificate: verifyCert };
		var fullCert = checkCertReply(checkCert);
	...
	case "PubKey":
		...
		signerPubKey = kitData(signerReply);
...
{
	console.log('Signer Error Returned: '+ error);
}
...
function registerSigner()
{
	...
	{
		// Alert and return
		alert("Could not retrieve registration code. Please try again.");
		return;
	}
